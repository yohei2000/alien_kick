import {
  BALL_LIFE,
  BEAT_SECONDS,
  CALIBRATED_INPUT_OFFSET_SECONDS,
  FIRST_HIT_BEAT,
  GOOD_WINDOW,
  HIT_WINDOW,
  MUSIC_BPM,
  MUSIC_STEP_SECONDS,
  NOTE_HIGHWAY_PREVIEW_SECONDS,
  PERFECT_WINDOW,
  RHYTHM_PATTERN_BEATS,
  collectRhythmTimeline,
  getRhythmHitAtMusicBeat,
  makeRhythmCue,
  rhythmPatterns,
} from "./rhythm-data.js";
...
const SFX_GAIN = 0.72;
const DRUM_BUS_GAIN = 0.92;
const HARMONY_BUS_GAIN = 0.74;
const LEAD_BUS_GAIN = 0.82;
const ALIEN_BUS_GAIN = 0.7;
const RHYTHM_BUS_GAIN = 0.88;
...
function createRhythmAudit() {
  const events = [];
  const add = (type, data = {}) => {
    events.push({
      type,
      atMs: Math.round(performance.now() * 1000) / 1000,
      songTime: typeof state?.beat === "number" ? roundTime(state.beat) : 0,
      ...data,
    });
  };
  return {
...
class KickScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.layout();
    syncHud();
    if (auditMode && query.get("autostart") !== "0") {
...
function resetGame() {
  ...
  playStartFanfare();
  playAlienIntroSfx(aliens[state.alienIndex]);
  recordRhythmAudit("start", { bpm: MUSIC_BPM, firstHitBeat: FIRST_HIT_BEAT, firstHitTime: FIRST_HIT_BEAT * BEAT_SECONDS });
  syncHud();
  message.hidden = true;
}
...
function setupAudio() {
  ...
  state.drumGain = audio.createGain();
  state.harmonyGain = audio.createGain();
  state.leadGain = audio.createGain();
  state.alienGain = audio.createGain();
  state.rhythmGain = audio.createGain();
  ...
  state.drumGain.gain.value = DRUM_BUS_GAIN;
  state.harmonyGain.gain.value = HARMONY_BUS_GAIN;
  state.leadGain.gain.value = LEAD_BUS_GAIN;
  state.alienGain.gain.value = ALIEN_BUS_GAIN;
  state.rhythmGain.gain.value = RHYTHM_BUS_GAIN;
  ...
  state.drumGain.connect(state.musicGain);
  state.harmonyGain.connect(state.musicGain);
  state.leadGain.connect(state.musicGain);
  state.alienGain.connect(state.musicGain);
  state.rhythmGain.connect(state.musicGain);
...
function getSongTime() {
  const audio = state.audio;
  if (!audio || !state.audioReady || audio.state !== "running") return state.beat;
...
function getAudibleSongTime() {
  return Math.max(0, getSongTime() - getAudioLatencyCompensation());
}

function getVisualSongTime() {
  return getAudibleSongTime();
}

function getInputSongTime() {
  return getAudibleSongTime() + CALIBRATED_INPUT_OFFSET_SECONDS;
}
...
function scheduleTone({
  ...
  const panner = pan && audio.createStereoPanner ? audio.createStereoPanner() : null;
  osc.type = type;
  ...
  if (panner) {
    panner.pan.setValueAtTime(panValue(pan), start);
    amp.connect(panner);
    panner.connect(bus);
  } else {
    amp.connect(bus);
...
function queueNextRhythmCue() {
  state.nextRhythmCue = makeRhythmCue({
    patternIndex: state.patternIndex,
    rhythmStep: state.rhythmStep,
    patternStartBeat: state.patternStartBeat,
  });
}
...
function updateMusic() {
  ...
  const duck = getMusicDuck(songNow);
  state.musicGain?.gain.setTargetAtTime(MUSIC_GAIN + energy * 0.045, audio.currentTime, 0.08);
  state.drumGain?.gain.setTargetAtTime(DRUM_BUS_GAIN + energy * 0.04, audio.currentTime, 0.08);
  state.harmonyGain?.gain.setTargetAtTime((HARMONY_BUS_GAIN + energy * 0.045) * (1 - duck * 0.72), audio.currentTime, 0.05);
  state.leadGain?.gain.setTargetAtTime((LEAD_BUS_GAIN + energy * 0.04) * (1 - duck * 0.42), audio.currentTime, 0.05);
  state.alienGain?.gain.setTargetAtTime((ALIEN_BUS_GAIN + energy * 0.035) * (1 - duck * 0.55), audio.currentTime, 0.05);
  state.rhythmGain?.gain.setTargetAtTime(RHYTHM_BUS_GAIN + energy * 0.03, audio.currentTime, 0.04);
...
function scheduleMusicStep(stepIndex) {
  ...
  const oneBeatCue = getRhythmHitAtMusicBeat(beat, 1);
  const halfBeatCue = getRhythmHitAtMusicBeat(beat, 0.5);
  recordRhythmAudit("music-step", { stepIndex, songTime: roundTime(songTime), beat: roundTime(beat) });
  ...
  if (oneBeatCue) schedulePreTargetCue(start, oneBeatCue, 1, energy, songTime);
  if (halfBeatCue) schedulePreTargetCue(start, halfBeatCue, 0.5, energy, songTime);
  if (hit) scheduleTargetTick(start, hit, songTime);
}
...
function duckMusic(depth = 0.24, duration = 0.18) {
  if (!state.audioReady) return;
  const now = getSongTime();
  state.duckStarted = now;
  state.duckUntil = Math.max(state.duckUntil, now + duration);
  state.duckDepth = Math.max(state.duckDepth, depth);
  state.duckDuration = Math.max(0.001, duration);
}

function getMusicDuck(songNow) {
  if (state.duckUntil <= songNow) {
    state.duckDepth = 0;
    return 0;
  }
  const remain = state.duckUntil - songNow;
  const amount = Math.min(1, remain / Math.max(0.001, state.duckDuration));
  return state.duckDepth * amount;
}
...
function collectRhythmGuideNotes(scene) {
  ...
  const timeline = collectRhythmTimeline({
    fromBeat: Math.max(FIRST_HIT_BEAT, nowMusicBeat - HIT_WINDOW / BEAT_SECONDS * 1.7),
    toBeat: lastMusicBeat,
  });

  for (const hit of timeline) {
    const timeToHit = hit.hitTime - state.beat;
    if (timeToHit > NOTE_HIGHWAY_PREVIEW_SECONDS + 0.05 || timeToHit < -HIT_WINDOW * 1.6) continue;
    const ball = scene?.balls.find(
      (entry) => Math.abs(entry.hitTime - hit.hitTime) < 0.012 && entry.lane === hit.lane && entry.side === hit.side,
    );
...
function scheduleKickDrum(start, strength = 1) {
  ...
  scheduleNoise({ start, duration: 0.035, gain: 0.035 * strength, bus: state.drumGain, filterType: "lowpass", freq: 420, q: 0.4 });
}

function scheduleSnareDrum(start, strength = 1) {
  scheduleNoise({ start, duration: 0.09, gain: 0.09 * strength, bus: state.drumGain, filterType: "bandpass", freq: 1900, q: 0.9 });
  scheduleTone({
    freq: 185,
    start,
    duration: 0.06,
...
function scheduleHat(start, strength = 1) {
  scheduleNoise({
    ...
    freq: 7200,
    q: 0.7,
    pan: Math.random() > 0.5 ? 0.16 : -0.16,
  });
}

function scheduleOpenHat(start, strength = 1) {
  scheduleNoise({ start, duration: 0.14, gain: 0.026 * strength, bus: state.drumGain, filterType: "highpass", freq: 6100, q: 0.7, pan: 0.22 });
}
...
function schedulePreTargetCue(start, hit, leadBeats, energy, cueSongTime = hit.hitTime - leadBeats * BEAT_SECONDS) {
  const pan = (hit.lane - 1) * 0.36;
  const laneNotes = [NOTE.C5, NOTE.E5, NOTE.G5];
  const closeCue = leadBeats < 1;
  const freq = laneNotes[hit.lane] * (leadBeats >= 1 ? 0.5 : 1);
  recordRhythmAudit("pre-cue", {
    lane: hit.lane,
    leadBeats,
    cueTime: roundTime(cueSongTime),
    hitTime: roundTime(hit.hitTime),
    diff: roundTime(cueSongTime - (hit.hitTime - leadBeats * BEAT_SECONDS)),
  });
  scheduleTone({
    freq,
    start,
    duration: leadBeats >= 1 ? 0.07 : 0.095,
    gain: (leadBeats >= 1 ? 0.044 : 0.072) + energy * 0.006,
    type: closeCue ? "triangle" : "sine",
    bus: state.rhythmGain,
    attack: 0.002,
    release: 0.055,
    filterType: "highpass",
    filterFreq: closeCue ? 1500 + energy * 260 : 980 + energy * 220,
    pan,
    send: leadBeats >= 1 ? 0.16 : 0.24,
  });
  if (closeCue) {
    scheduleNoise({
      start,
      duration: 0.025,
      gain: 0.016 + energy * 0.002,
      bus: state.rhythmGain,
      filterType: "highpass",
      freq: 5400,
      pan,
...
function scheduleTargetTick(start, hit, targetSongTime = hit.hitTime) {
  const laneNotes = [NOTE.C5, NOTE.E5, NOTE.G5];
  const root = laneNotes[hit.lane] * (hit.accent ? 2.1 : 1.55);
  const pan = (hit.lane - 1) * 0.42;
  recordRhythmAudit("target-tick", {
    lane: hit.lane,
    hitTime: roundTime(hit.hitTime),
    targetTickTime: roundTime(targetSongTime),
    diff: roundTime(targetSongTime - hit.hitTime),
  });
  scheduleKickDrum(start, hit.accent ? 0.74 : 0.52);
  scheduleTone({
    freq: 118,
    sweepTo: 64,
    start,
    duration: 0.075,
    gain: hit.accent ? 0.09 : 0.062,
    type: "sine",
    bus: state.rhythmGain,
    attack: 0.0015,
    release: 0.038,
  ...
  scheduleTone({
    ...
    start,
    duration: 0.074,
    gain: hit.accent ? 0.12 : 0.084,
    type: "triangle",
    bus: state.rhythmGain,
    attack: 0.002,
    release: 0.052,
    pan,
    send: hit.accent ? 0.3 : 0.18,
  });
  scheduleNoise({
    start,
    duration: 0.038,
    gain: hit.accent ? 0.046 : 0.032,
    bus: state.rhythmGain,
    filterType: "highpass",
    freq: 7200,
    pan,
...
function playIncomingCue(ball) {
  ...
  const pan = (ball.lane - 1) * 0.35;
  recordRhythmAudit("spawn-sfx", {
    lane: ball.lane,
    spawnTime: roundTime(ball.born),
    hitTime: roundTime(ball.hitTime),
  });
  scheduleNoise({
    start,
    duration: 0.16,
    gain: ball.accent ? 0.018 : 0.012,
    bus: state.sfxGain,
    filterType: "bandpass",
    freq: 1700 + ball.lane * 280,
    q: 1.2,
    pan,
    send: 0.08,
  });
  scheduleTone({
    freq: 220 + ball.lane * 42,
    sweepTo: 150 + ball.lane * 36,
    start,
    duration: 0.11,
    gain: ball.accent ? 0.012 : 0.008,
    type: "sine",
    bus: state.sfxGain,
    attack: 0.006,
    release: 0.08,
...
function playStartFanfare() {
  const audio = state.audio;
  if (!audio) return;
  const start = state.musicStartAt || audio.currentTime;
  scheduleCountIn(start);
  scheduleKickDrum(start, 1.18);
  [NOTE.A3, NOTE.E4, NOTE.A4, NOTE.C5, NOTE.E5].forEach((freq, i) => {
    scheduleTone({
      freq,
      start: start + i * BEAT_SECONDS * 0.5,
      duration: i === 4 ? BEAT_SECONDS * 0.62 : BEAT_SECONDS * 0.34,
      gain: i === 4 ? 0.056 : 0.04,
      type: i % 2 ? "triangle" : "square",
      bus: i >= 3 ? state.rhythmGain : state.leadGain,
      attack: 0.004,
      release: 0.12,
      filterType: "lowpass",
  ...
  scheduleNoise({ start: start + BEAT_SECONDS * 1.9, duration: 0.18, gain: 0.035, bus: state.drumGain, filterType: "highpass", freq: 5200, pan: 0.2, send: 0.16 });
}

function scheduleCountIn(start) {
  [0, 1, 2, 3].forEach((beat) => {
    const cueStart = start + beat * BEAT_SECONDS;
    const strong = beat === 3;
    recordRhythmAudit("count-in", { beat, cueTime: roundTime(beat * BEAT_SECONDS) });
    scheduleTone({
      freq: strong ? NOTE.A5 : NOTE.E5,
      start: cueStart,
      duration: strong ? 0.095 : 0.07,
    ...
    scheduleNoise({
      start: cueStart,
      duration: strong ? 0.036 : 0.024,
      gain: strong ? 0.024 : 0.014,
      bus: state.rhythmGain,
      filterType: "highpass",
      freq: strong ? 7200 : 5600,
    });
  });
}

function playAlienIntroSfx(alien) {
  const audio = state.audio;
  if (!audio || !alien) return;
  const start = audio.currentTime + 0.08;
  if (alien.type === "Slime") {
    [NOTE.C3, NOTE.G2, NOTE.C3].forEach((freq, i) => {
      scheduleTone({
        freq,
        sweepTo: freq * 0.64,
        start: start + i * 0.08,
  ...
  } else if (alien.type === "Mantis") {
    [NOTE.D5, NOTE.A4, NOTE.F5].forEach((freq, i) => {
      scheduleTone({
        freq,
        start: start + i * 0.055,
        duration: 0.08,
    ...
    scheduleNoise({ start: start + 0.12, duration: 0.07, gain: 0.034, bus: state.alienGain, filterType: "bandpass", freq: 3300, q: 2.4, pan: 0.36 });
  } else {
    [NOTE.E4, NOTE.B4, NOTE.E5].forEach((freq, i) => {
      scheduleTone({
        freq,
        start: start + i * 0.11,
        duration: 0.34,
...
function playKickImpactSfx(timing) {
  ...
  recordRhythmAudit("kick-sfx", { timing, audioStart: roundTime(start), songTime: roundTime(getSongTime()) });
  duckMusic(hard ? 0.36 : 0.22, hard ? 0.24 : 0.16);
...
function playGoalSfx(timing) {
  ...
  const start = audio.currentTime;
  const hard = timing === "hard";
  duckMusic(hard ? 0.28 : 0.18, hard ? 0.3 : 0.2);
  const notes = hard ? [659.25, 880, 1174.66] : [523.25, 659.25, 783.99];
  notes.forEach((freq, i) => {
    scheduleTone({ freq, start: start + i * 0.035, duration: 0.12, gain: hard ? 0.075 : 0.052, type: "triangle", bus: state.sfxGain, release: 0.08, pan: (i - 1) * 0.22, send: hard ? 0.2 : 0.12 });
  });
...
function spawnBall(cue) {
  ...
  recordRhythmAudit("spawn", {
    id: ball.id,
    lane,
    hitTime: roundTime(ball.hitTime),
    spawnTime: roundTime(ball.born),
    expectedSpawnTime: roundTime(ball.hitTime - BALL_LIFE),
    diff: roundTime(ball.born - (ball.hitTime - BALL_LIFE)),
  });
...
function kickAt(clientX) {
  ...
  if (!active || active.diff > HIT_WINDOW) {
    ...
    recordRhythmAudit("miss", {
      reason: earlyLate,
      signedDiff: roundTime(active?.signedDiff || -HIT_WINDOW),
      inputSongTime: roundTime(inputSongTime),
      hitTime: active ? roundTime(active.ball.hitTime) : null,
    });
  ...
  recordRhythmAudit("input", {
    id: active.ball.id,
    tapLane,
    noteLane: active.ball.lane,
    laneMatched: active.ball.laneMatched,
    hitTime: roundTime(active.ball.hitTime),
    inputSongTime: roundTime(inputSongTime),
    signedDiff: roundTime(active.signedDiff),
    absDiff: roundTime(active.diff),
    timing,
  });
...
function updateAuditAutotap() {
  ...
  state.autoTapDone.add(target.ball.id);
  const guide = rhythmGuideLayout();
  const lane = autoTapMode === "wrongLane" ? (target.ball.lane + 1) % guide.laneXs.length : target.ball.lane;
  recordRhythmAudit("autotap", {
    id: target.ball.id,
    mode: autoTapMode,
    lane,
...
function updateGame(dt) {
  ...
  state.beat = songNow;
  state.songTimeSeconds = songNow;
  state.musicBeat = songNow / BEAT_SECONDS;
  ...
  for (const ball of sceneRef.balls) {
    if (!ball.hit && !ball.missed && state.beat > ball.hitTime + HIT_WINDOW) {
      ...
      recordRhythmAudit("miss", {
        ...
        lane: ball.lane,
        hitTime: roundTime(ball.hitTime),
        songTime: roundTime(state.beat),
        signedDiff: roundTime(HIT_WINDOW),
      });
...
function getKickerCue() {
  ...
  if (active.timeToHit <= BEAT_SECONDS * 0.75) return "charge";
  if (active.timeToHit <= BEAT_SECONDS * 1.5) return "aim";
  return "idle";
}
